...
d();
const l = window.matchMedia("(prefers-color-scheme: dark)") ? "dark" : "light";
localStorage.getItem("theme") === null && localStorage.setItem("theme", l);
const c = localStorage.getItem("theme") || l;
...
function g() {
  localStorage.getItem("theme") === "dark" ? s("light") : s("dark");
}
window.toggleTheme = g;
s(c);
const i = document.getElementById("scroll-to-top");
window.onscroll = h;
function h() {
...
const u = document.getElementById("obj-rocket");
window.onload = () => {
  setTimeout(() => {
    u.style.transform = "translate(0,0)";
  }, 1e3);
};
